...
function categories()
{
    $mysqli = new mysqli("localhost", "root", "", "biddingsystemdb");
...
// This is the duplicate declaration of the categories() function.
//function categories() {
//    echo "This is the duplicate declaration of the categories() function.";
//}
...
function latest()
{
    ...
    $sql = "SELECT * FROM products ORDER BY productid DESC LIMIT 0,6";
    $result = $mysqli->query($sql);
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<div class='prod_price'><span>Start Bid at: </span> <span class='price'>P " . $row['startingbid'] . "</span><br />
            ";
            // echo "<span>Highest Bidder: </span> <span class='price'>".$row['highestbidder']."</span>";
            echo "</div>";
            echo "</div>";
            echo "<div class='bottom_prod_box'></div>";
            echo "<div class='prod_details_tab'><a href='details.html' class='prod_details' title='header=[Click to Bid] body=[&nbsp;] fade=[on]'>Bid Now</a> </div>";
            echo "</div>";
        }
    } else {
...
function logform(){
		...
		While($rows = mysql_fetch_array($query)){
			echo '<div class="title_box">Welcome</div>
					<div class="border_box">
							...
							<strong>Bid Counter:</strong> 1<br /><br />
							<strong>Items Acquired:</strong> 1<br /><br />
	...
	// update the needtopay table when bidding time expires
	function updneedtopay(){
		...
		while($row = mysql_fetch_array($query))
		{
			...
			// highest bidder - member id is : $hibidder and name is : $highname
			// product id is : $prodid
			// highest bidding amount is: $highbid
			mysql_query("UPDATE products SET status = 1 WHERE productid = '$prodid'");
			mysql_query("UPDATE bidreport SET status = 1 WHERE productid = '$prodid' and bidder = '$hibidder' and bidamount = '$highbid'");
			mysql_query("INSERT INTO needtopay(memberid, productid, status, payment, declined, dateadded) 
						VALUES ('$hibidder','$prodid',0,'$highbid',0,CURDATE())");
		}
	  }
