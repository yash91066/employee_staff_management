...
Class Master extends DBConnection {
    ...
    if(!ctype_alpha($name)){
        $resp['status'] = 'failed';
        $resp['title'] = 'Invalid Input';
        $resp['msg'] = 'Please Enter Valid Name Using Alphabets Only.';
        return json_encode($resp);
    }
    ...
    if(!ctype_digit($contact)){
        $resp['status'] = 'failed';
        $resp['title'] = 'Invalid Input';
        $resp['msg'] = 'Please Enter Numbers Only.';
        return json_encode($resp);
    }
    ...
    if(!ctype_alnum($address)){
        $resp['status'] = 'failed';
        $resp['title'] = 'Invalid Input';
        $resp['msg'] = 'Please Enter Valid Address.';
        return json_encode($resp);
    }
	// Validate purpose field
    if(!ctype_alpha($purpose)){
        $resp['status'] = 'failed';
        $resp['title'] = 'Invalid Input';
        $resp['msg'] = 'Please Enter Valid Alphabets Only.';
        return json_encode($resp);
    }
	...
	//
		...
		if($save){
			$resp['status'] = 'success';
			if($type == 1){
			$resp['title'] = 'Sucessfully Clock In';
			$resp['msg'] = 'Welcome '. $name;
		}else{
			$resp['title'] = 'Sucessfully Clock Out';
			$resp['msg'] = 'Goodbye '. $name;
		}
		}else{
			$resp['status'] = 'failed';
			$resp['title'] = 'Clock In Error.Please Try Again';
		}
		return json_encode($resp);
	}
}
...
switch ($action) {
	...
	function log_employee(){
		...
		if($qry->num_rows > 0){
			...
			$resp['status'] = 'failed';
			$resp['title'] = 'Please Enter Correct Employee Code';
			$resp['msg'] = '';
		}
		return json_encode($resp);
	}
